...
import { ChevronLeft, ChevronRight, Send, Pencil, EyeOff } from 'lucide-react';
...
import { resumeDe, type Revision } from '../historique';
...
export function BookDetailPage() {
  ...
  return (
    <div className="mx-auto w-full max-w-2xl px-4 pb-8 lg:max-w-4xl lg:px-8">
      ...
      {/* Seulement si le livre a bougé : proposer d'ouvrir un journal vide sur
          un brouillon qui vient de naître ferait une promesse creuse. */}
      {aEteModifie(book.createdAt, book.updatedAt) && <CeQuiAChange bookId={book.id} />}
 ...
 *
 * Un auteur ouvre cette fiche pour savoir où en est son livre, pas pour lire
 * son journal de bord : la rubrique est donc fermée, et ne coûte rien tant
 * qu'on ne l'ouvre pas — c'est aussi pour cela que la requête n'est lancée
 * qu'à l'ouverture. Elle répond quand on la pose : « ai-je bien enregistré ma
 ...
 */
function CeQuiAChange({ bookId }: { bookId: string }) {
  ...
  async function charger() {
    ...
    try {
      const res = await getBookRevisions(bookId, { limit: 5 });
  ...
  return (
    <section className="mt-8">
      <details
        className="group"
        onToggle={(e) => {
          if ((e.currentTarget as HTMLDetailsElement).open) void charger();
        }}
      >
        ...
        <div className="mt-2 pl-[22px]">
          {erreur ? (
            ...
            <p className="text-sm text-on-surface-muted">
              Rien depuis la création de ce livre.
            </p>
          ) : (
