...
export function BookDetailPage() {
  ...
  return (
    <div className="mx-auto w-full max-w-2xl px-4 pb-8 lg:max-w-4xl lg:px-8">
      ...
      {aEteModifie(book.createdAt, book.updatedAt) && <CeQuiAChangeLivre bookId={book.id} />}
...
/** L'historique de ce livre : cinq lignes, chargées à l'ouverture. */
function CeQuiAChangeLivre({ bookId }: { bookId: string }) {
  return (
    <CeQuiAChange
      charger={async () => (await getBookRevisions(bookId, { limit: 5 })).data}
      vide="Rien depuis la création de ce livre."
    />
  );
}
